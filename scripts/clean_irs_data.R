...
# Directories
old_format_inflow <- c('inflow/countyinflow0607.csv',
                       'inflow/countyinflow0708.csv',
                       'inflow/countyinflow0809.csv',
...
old_format_inflows <- old_format_inflow |> 
  ...
  map(.f = ~read.csv(.x) |> # format Hell
        filter(str_c(State_Code_Origin, County_Code_Origin) != 
                 str_c(State_Code_Dest, County_Code_Dest)) |>
        mutate(State_Code_Dest = str_pad(State_Code_Dest, 2, 'left', '0'),
                   ...
                   Return_Num = str_remove_all(Return_Num, ","),
                   Exmpt_Num  = str_remove_all(Exmpt_Num, ","),
                   Aggr_AGI   = str_remove_all(Aggr_AGI, ",")) |>
        select(d_fips, Return_Num, Exmpt_Num, Aggr_AGI) |>
        group_by(d_fips) |>
        summarise(n_return_in = sum(as.numeric(Return_Num), na.rm=TRUE),
                  n_exempt_in = sum(as.numeric(Exmpt_Num), na.rm=TRUE),
                  agi = sum(as.numeric(Aggr_AGI), na.rm=TRUE)))

new_format_inflows <- new_format_inflow |>
  map(.f = ~str_c(PREFIX, .x)) |>
  map(.f = ~read.csv(.x) |>
        filter(str_c(y1_statefips, y1_countyfips) != 
                 str_c(y2_statefips, y2_countyfips)) |>  
...
# Directories
old_format_outflow <- c('outflow/countyoutflow0607.csv',
                        'outflow/countyoutflow0708.csv',
                        'outflow/countyoutflow0809.csv',
...
old_format_outflows <- old_format_outflow |> 
  ...
  map(.f = ~read.csv(.x) |> 
        ...
        group_by(o_fips) |>
        summarise(n_return_out = sum(as.numeric(Return_Num), na.rm=TRUE),
                  n_exempt_out = sum(as.numeric(Exmpt_Num), na.rm=TRUE),
                  agi = sum(as.numeric(Aggr_AGI), na.rm=TRUE)))
...
write.csv(outflows, '../data/irs_outflows.csv', row.names=FALSE)
write.csv(inflows, '../data/irs_inflows.csv', row.names=FALSE)
